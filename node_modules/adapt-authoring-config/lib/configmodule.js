...
class ConfigModule extends AbstractModule {
  ...
  */
  async storeUserSettings() {
    const c = await ConfigUtils.loadConfigFile(path.join(process.cwd(), 'conf', `${this.get('env.NODE_ENV')}.config.js`));
    if(!c) {
      return;
    }
  ...
  */
  storeSchemaSettings() {
    ...
    return Promise.all(Object.values(this.app.dependencies).map(d => this.processModuleSchema(d)));
  }
  /**
  ...
  */
  async processModuleSchema(pkg) {
    if(!pkg.name || !pkg.rootDir) {
      return;
    }
    const jsonchema = await this.app.waitFor('jsonschema');
