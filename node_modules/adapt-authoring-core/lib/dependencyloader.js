  ...
  */
  async load() {
    ...
    for(let i = 0, count = installedMods.length; i < count; i++) {
      ...
      try {
        ...
        console.log('ERROR', getModName(modRoot), e);
        hasFailed = true;
      }
    }
    console.log(this.instances);
    if(hasFailed) {
      this.throwError('error.loadfailed');
    }
    return;
  }
  async loadModule(modRoot) {
    ...
    try {
      ...
      this.throwError('error.importmodule');
    }
    if(ModClass.Module) {
      ModClass = ModClass.Module;
    }
    if(!Utils.isFunction(ModClass)) {
      this.throwError('error.expectedclass');
    }
    try {
      instance = new ModClass(this.app, modConfig);
    } catch(e) {
      this.throwError('error.createmodule');
    }
    if(!Utils.isFunction(instance.onReady)) {
      this.throwError('error.expectedonready');
    }
