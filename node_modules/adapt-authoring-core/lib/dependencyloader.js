...
class DependencyLoader extends Events {
  ...
  */
  async load() {
    ...
    await Promise.all(coreApis.map(d => this.loadModule(d.name)));
    // load all remaining modules
    return Promise.all(theRest.map(d => this.loadModule(d.name)));
  }
  /**
  *
  */
  async loadModule(modName) {
    console.log('DependencyLoader#loadModule', modName);
    if(this.instances[modName]) {
      return this.handleError('error.modulealreadyexists', { name: modName });
    }
    ...
    try {
      ...
      return this.handleError('error.importmodule', { filepath: config.rootDir });
    }
    if(ModClass.Module) {
      ModClass = ModClass.Module;
    }
    if(!Utils.isFunction(ModClass)) {
      return this.handleError('error.expectedclass', { name: modName });
    }
    try {
      instance = new ModClass(this.app, config);
    } catch(e) {
      console.log(e);
      return this.handleError('error.createmodule', { name: modName });
    }
    if(!Utils.isFunction(instance.onReady)) {
      return this.handleError('error.expectedonready', { name: modName });
    }
  ...
  */
  async loadModuleConfig(modName) {
    ...
    try {
      ...
      this.handleError(`Failed to load module config for ${modPath}, ${e}`);
    }
  }
  /**
  *
  */
  async waitFor(modName) {
    const m = this.instances[modName] || this.instances[`adapt-authoring-${modName}`];
    if(!m) {
      this.handleError('error.missingmodule', { name: modName });
    }
    return m.onReady();
  }
  /**
  ...
  */
  handleError(key, data, type = 'error') {
    this.emit('loaderror', new DependencyLoaderError(key, data, type));
  }
}
/**
