...
class DependencyLoader extends Events {
  ...
  */
  async load() {
    ...
    try {
      ...
      coreApis.forEach(m => this.app[m.coreApiName] = this.instances[m.name]);
  ...
  */
  async waitFor(modName) {
    const exists = this.dependencies[modName] || this.dependencies[`adapt-authoring-${modName}`];
    if(!exists) {
      this.throwError('error.missingmodule', { name: modName });
    }
    const instance = this.instances[modName] || this.instances[`adapt-authoring-${modName}`];
    if(instance) return instance.onReady();

    this.on('moduleloaded', (name, instance) => {
      if(name === modName) {
