  ...
  */
  async loadModule(modName) {
    ...
    if(this.instances[modName]) {
      return this.throwError('error.modulealreadyexists', { name: modName });
    }
    ...
    try {
      ...
      return this.throwError('error.importmodule', { filepath: config.rootDir });
    }
    if(ModClass.Module) {
      ModClass = ModClass.Module;
    }
    if(!Utils.isFunction(ModClass)) {
      return this.throwError('error.expectedclass', { name: modName });
    }
    try {
      instance = new ModClass(this.app, config);
    } catch(e) {
      return this.throwError('error.createmodule', { name: modName });
    }
    if(!Utils.isFunction(instance.onReady)) {
      return this.throwError('error.expectedonready', { name: modName });
    }
